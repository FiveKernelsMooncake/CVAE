...
class CVAE:
    ...
    def cdl_estimate(self, data_x, num_iter):
        for i in range(num_iter):
            ...
            _, l, gen_loss, v_loss = self.sess.run((self.optimizer, self.loss, self.gen_loss, self.v_loss),feed_dict={self.x: b_x, self.v: self.m_V[ids, :]})
    ...
    def pmf_estimate(self, users, items, test_users, test_items, params):
        ...
        while ((it < params.max_iter and converge > 1e-6) or it < min_iter):
            ...
            for i in range(self.m_num_users):
                ...
                if n > 0:
                    A = np.copy(XX)
                    A = A+np.dot(self.m_V[item_ids, :].T, self.m_V[item_ids,:])*a_minus_b
